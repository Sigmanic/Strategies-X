...
return function(self, p1)
	...
	local NameTable = MapName..":"..ModeName
	ElevatorSettings[ModeName].Enabled = true
	...
	task.spawn(function()
		if CheckPlace() then
			...
			ConsoleInfo("Map Selected: "..GameMapName..", ".."Mode: "..ModeName..", ".."Solo Only: "..tostring(Solo))
			return
		end
		local Elevators = {
			["Survival"] = {},
		...
		for i,v in next,Workspace.Elevators:GetChildren() do
			if SpecialGameMode[MapName] then
				...
				return
			elseif game:GetService("MarketplaceService"):UserOwnsGamePassAsync(LocalPlayer.UserId, 10518590) or UtilitiesConfig.PreferMatchmaking then
			...
			if not Passed then
				ElevatorType = v:GetAttribute("Type") --ElevatorType = if v:GetAttribute("Level") == 50 then "Hardcore" else "Survival"
			end
		...
		while true do
			...
			for Name, Check in next, ElevatorSettings do
                ...
                --Changing Elevators [[NO LONGER EXIST]]
						...
						for i,v in ipairs(Elevators[Name]) do
							...
							if MapTable and Playing < 4 then
								...
								MapGlobal.ConnectionEvent = v["Object"]:GetAttributeChangedSignal("Timer"):Connect(function()
									local Time = v["Object"]:GetAttribute("Timer")
									local Map = v["Object"]:GetAttribute("Map")
									local Playing = v["Object"]:GetAttribute("Players")
									local Mode = v["Mode"]
									local MapTableName = Map..":"..Mode
									UI.MapFind.Text = "Map: "..Map
									UI.CurrentPlayer.Text = "Player Joined: "..Playing
									UI.TimerLeft.Text = "Time Left: "..tostring(Time)
									prints("Time Left: ",Time)
									...
									if Time > 0 and (not MapGlobal[MapTableName] or (MapGlobal[MapTableName].Solo and Playing > 1)) then
